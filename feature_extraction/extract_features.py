...
def load_frame(frame_file, resize=False):
    ...
    # FIX 1: bỏ assert kích thước cứng 256x340 — ảnh của bạn là 256x256
    if resize:
        data = data.resize((224, 224), Image.LANCZOS)  # FIX 2: ANTIALIAS deprecated
...
def load_zipframe(zipdata, name, resize=False):
    ...
    # FIX 1: bỏ assert kích thước cứng
    if resize:
        data = data.resize((224, 224), Image.LANCZOS)  # FIX 2
...
def load_rgb_batch(frames_dir, rgb_files, frame_indices, resize=False):
    if resize:
        ...
        # FIX 3: dùng 256x256 thay vì 256x340
        batch_data = np.zeros(frame_indices.shape + (256, 256, 3))
...
def load_flow_batch(frames_dir, flow_x_files, flow_y_files, frame_indices, resize=False):
    if resize:
        batch_data = np.zeros(frame_indices.shape + (224, 224, 2))
    else:
        batch_data = np.zeros(frame_indices.shape + (256, 256, 2))
...
def load_zipflow_batch(flow_x_zipdata, flow_y_zipdata,
                       flow_x_files, flow_y_files, frame_indices, resize=False):
    if resize:
        batch_data = np.zeros(frame_indices.shape + (224, 224, 2))
    else:
        batch_data = np.zeros(frame_indices.shape + (256, 256, 2))
...
def run(mode='rgb', load_model='', sample_mode='oversample', frequency=16,
    ...
    def forward_batch(b_data):
        ...
        with torch.no_grad():  # FIX 4: thay Variable volatile=True (deprecated)
    ...
    print(f"Tìm thấy {len(video_names)} video")
    ...
    for video_name in video_names:
        ...
        if frame_cnt <= chunk_size:
            print(f"[BỎ QUA] {video_name}: chỉ có {frame_cnt} frames")
            continue
        ...
        for batch_id in range(batch_num):
            ...
            if mode == 'rgb':
                ...
                if usezip:
                    batch_data = load_zipflow_batch(flow_x_zipdata, flow_y_zipdata,
